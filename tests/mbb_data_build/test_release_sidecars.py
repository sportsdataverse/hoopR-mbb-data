...
from mbb_data_build.config import PKG_FUNCTION, REGISTRY
...
def test_stamp_uploads_the_four_sidecars_with_clobber():
    ...
    publish._stamp(
        "espn_mens_college_basketball_game_rosters",
        calls.append,
    ...
    assert all(
        c[:3] == ["release", "upload", "espn_mens_college_basketball_game_rosters"] for c in calls
    )
...
def test_stamp_names_the_loader_for_the_tag():
    ...
    publish._stamp(
        "espn_mens_college_basketball_game_rosters", _runner, "sportsdataverse/sportsdataverse-data"
    )

    expected = PKG_FUNCTION["espn_mens_college_basketball_game_rosters"]
