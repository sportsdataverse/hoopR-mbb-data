...
(``R/espn_mbb_NN_<key>_creation.R``), Python is a build package with datasets
as ``config.REGISTRY`` rows. The numbered shims in ``python/`` bridge that:
``python/espn_mbb_NN_<key>_creation.py`` carries the SAME number as its R twin.
...
CONFIG = REPO / "python" / "mbb_data_build" / "config.py"

_R_STAGE = re.compile(r"^(?:espn_)?mbb_(?P<num>\d{2})_(?P<key>.+)_creation$")
_PY_STAGE = re.compile(r"^espn_mbb_(?P<num>\d{2})_(?P<key>.+)_creation$")
...
KNOWN_UNPAIRED: dict[str, str] = {
    "schedules": (
        "R writes it inside espn_mbb_01_pbp_creation.R (verified: saveRDS to "
        "mbb/schedules/rds/) rather than as its own numbered stage. Later stages "
        "re-write it to stamp has_* flags."
    ),
    "shots": (
        "R writes it inside espn_mbb_01_pbp_creation.R (verified: saveRDS + "
        "write_parquet to mbb/shots/) rather than as its own numbered stage."
    ),
