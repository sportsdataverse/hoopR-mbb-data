...
``hoopR-mbb-raw`` checkout and compare full-frame against the real committed
oracle under ``hoopR-mbb-data/mbb/{dataset}/parquet/``. Building the season
once per test session (not once per test) keeps the suite from re-parsing
the season's worth of JSON for every dataset that shares an input (schedules/
shots/player_season_stats all read another dataset's just-built parquet).

MBB delta vs the hoopR-nba-data template: no ``draft`` dataset (college
basketball has no draft), so it is absent from the build order.
...
large; see ``mbb_data_build/ingest.py``).
"""
...
def _skip_if_no_raw_checkout() -> None:
    if not RAW_ROOT.is_dir():
        pytest.skip(f"sibling hoopR-mbb-raw checkout not found at {RAW_ROOT}")
    if not ORACLE_ROOT.is_dir():
        pytest.skip(f"oracle mbb/ tree not found at {ORACLE_ROOT}")
...
def built_base(tmp_path_factory) -> Path:
    ...
    base = tmp_path_factory.mktemp("mbb_2025_build")
