...
def test_publish_logs_upload_confirmations(tmp_path, capsys):
    ...
    assert "uploading mbb_schedule_2025.parquet" in out
    assert f"uploaded mbb_schedule_2025.csv -> {spec.tag} (asset 3/3)" in out
    assert "[INFO] mbb_data_build:" in out  # timestamped, labeled lines
...
def test_publish_logs_the_manifest_upload(tmp_path, capsys):
    ...
    out = capsys.readouterr().out
    assert f"uploaded mbb_standings_in_data_repo.csv -> {spec.tag} (asset 4/4)" in out
