"""Read the hoopR-mbb-raw tree — from a sibling checkout OR over HTTP.
...
``{raw_root}/mbb/json/final/{game_id}.json``; the per-season schedule at
``{raw_root}/mbb/schedules/parquet/mbb_schedule_{season}.parquet``.

HTTP mode details: per-game/per-entity JSON is cached under
``$HOOPR_MBB_CACHE`` (default ``.mbb_raw_cache``, gitignored) so the dataset
...
def _resolve_root(explicit: str | Path | None) -> Path | str:
    """Resolve the hoopR-mbb-raw root (arg > env): a local Path or a base URL."""
    val = explicit or os.environ.get(RAW_ROOT_ENV)
    if not val:
        raise RuntimeError(
            f"set {RAW_ROOT_ENV} to the hoopR-mbb-raw checkout root or its "
...
def _cache_root() -> Path:
    return Path(os.environ.get("HOOPR_MBB_CACHE", ".mbb_raw_cache"))
...
def _read_season_schedule(season: int, root: Path | str) -> pl.DataFrame | None:
    ...
    rel = f"mbb/schedules/parquet/mbb_schedule_{season}.parquet"
...
def season_dir_ids(subdir: str, season: int, *, raw_root: str | Path | None = None) -> list[int]:
    """Numeric ids of the per-entity JSONs under ``mbb/{subdir}/json/{season}``.
    ...
    if isinstance(root, Path):
        d = root / "mbb" / subdir / "json" / str(season)
        if not d.is_dir():
            return []
        names = sorted(f.stem for f in d.glob("*.json"))
        return [int(n) for n in names if n.isdigit()]
    body = _http_get_bytes(f"{_RAW_REPO_API}/mbb/{subdir}/json/{season}")
    if body is None:
        return []
    try:
...
def flat_dir_ids(subdir: str, *, raw_root: str | Path | None = None) -> list[int]:
    """Numeric ids of the per-entity JSONs under the FLAT ``mbb/{subdir}/json``
    ...
    root = _resolve_root(raw_root)
    if isinstance(root, Path):
        d = root / "mbb" / subdir / "json"
        if not d.is_dir():
            return []
        names = sorted(f.stem for f in d.glob("*.json"))
        return [int(n) for n in names if n.isdigit()]
    body = _http_get_bytes(f"{_RAW_REPO_API}/mbb/{subdir}/json")
    if body is None:
        return []
    try:
        listing = json.loads(body)
...
def read_final(
    ...
    *,
    ...
    ``subdir`` selects the raw subtree under ``mbb/`` -- ``"json/final"``
    (default), ``"game_rosters/json"`` (also backs officials -- MBB has no
    ...
    payload under ``$HOOPR_MBB_CACHE``.
    """
    root = _resolve_root(raw_root)
    rel = f"mbb/{subdir}/{game_id}.json"
    if isinstance(root, Path):
        f = root / rel
        if not f.exists():
            return None
