"""Per-season build driver -- polars port of the R ``mbb_<dataset>_games(y)`` loop.
...
Build-order note (MBB delta): ``player_season_stats`` reads player_box's
...
def build_season(
    ...
    *,
    ...
    Args:
        ...
        raw_root: Sibling ``hoopR-mbb-raw`` checkout root (arg > ``HOOPR_MBB_RAW_ROOT`` env).
    ...
    if dataset not in reshapers.SEASON_BUILDERS and spec.reshaper not in reshapers.RESHAPERS:
        # Every dataset in REGISTRY now has a Python builder, so this is a
        # guard against a registry entry added without one -- not a standing
        # R carve-out. The .R creation scripts remain as the `-l R` rollback.
        raise NotImplementedError(
            f"{dataset}: no Python builder registered; see reshapers.SEASON_BUILDERS"
        )
    # All three crosswalks read LIVE ESPN/Fox/Torvik (+ sdv-py's bundled KenPom
    # directory), never the raw repo -- resolving the raw root for them would
    # make the build fail on a machine with no hoopR-mbb-raw checkout, for an
    # input they never open.
    ...
    if dataset in reshapers.SEASON_BUILDERS:
        ...
        log.info(
            "%s %s: season-level build starting (raw=%s via %s)",
            dataset,
            season,
            root,
            mode,
        ...
        if out.height == 0:
            log.warning(
                "%s %s: season-level build produced 0 rows; nothing written",
                dataset,
                season,
            )
            return out
        ...
        # NB: unlike NBA, MBB does NOT rebuild a master schedule / games-in-
        # data-repo extra here -- ``R/rebuild_mbb_master_schedule.R`` is a
        # maintainer-only interactive util, not part of the daily flow.
        log.info(
    ...
    if not game_ids:
        log.warning(
            "%s %s: no game_json games in the season schedule; nothing built",
            dataset,
            season,
        )
        return pl.DataFrame()
    ...
    if failed:
        log.warning(
            "%s %s: %d/%d games failed to reshape",
            dataset,
            season,
            failed,
            len(game_ids),
