...
Everything the pipeline reports flows through one ``mbb_data_build`` logger
writing to *stdout* (not stderr), because the daily processor tees stdout into
the per-season rotating logfile (``logs/hoopR_mbb_data_logfile_{season}.log``)
...
_FORMAT = "%(asctime)s [%(levelname)s] mbb_data_build: %(message)s"
...
def get_logger() -> logging.Logger:
    ...
    logger = logging.getLogger("mbb_data_build")
