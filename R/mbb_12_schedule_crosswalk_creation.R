...
build_season_schedule_crosswalk <- function(y) {
  cli::cli_progress_step(
    msg = "Compiling {y} MBB schedule crosswalk",
    msg_done = "Compiled {y} MBB schedule crosswalk!"
  )

  xwalk <- tryCatch(
    hoopR::mbb_schedule_crosswalk(season = y),
    error = function(e) {
  ...
  ifelse(
    !dir.exists(file.path("mbb/crosswalk")),
    dir.create(file.path("mbb/crosswalk"), recursive = TRUE),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("mbb/crosswalk/rds")),
    dir.create(file.path("mbb/crosswalk/rds"), recursive = TRUE),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("mbb/crosswalk/parquet")),
    dir.create(file.path("mbb/crosswalk/parquet"), recursive = TRUE),
    FALSE
  )

  saveRDS(xwalk, glue::glue("mbb/crosswalk/rds/mbb_schedule_crosswalk_{y}.rds"))
  arrow::write_parquet(
    xwalk,
    glue::glue("mbb/crosswalk/parquet/mbb_schedule_crosswalk_{y}.parquet"),
    compression = "zstd",
    compression_level = 22
  )

  cli::cli_progress_step(
    msg = "Updating {y} MBB Schedule Crosswalk GitHub Release",
    msg_done = "Updated {y} MBB Schedule Crosswalk GitHub Release!"
  )
  ...
  purrr::insistently(
    ...
    data_frame = xwalk,
    file_name = glue::glue("mbb_schedule_crosswalk_{y}"),
    sportsdataverse_type = "schedule crosswalk data",
    release_tag = "mbb_crosswalk",
    pkg_function = "hoopR::mbb_schedule_crosswalk()",
    file_types = c("rds", "csv", "parquet"),
    .token = Sys.getenv("GITHUB_PAT")
  )

  # --- Manifest row append --------------------------------------------------
  manifest_path <- "mbb/crosswalk/mbb_schedule_crosswalk_in_data_repo.csv"
  manifest_row <- tibble::tibble(
    season           = as.integer(y),
    row_count        = as.integer(nrow(xwalk)),
    generated_at_utc = format(Sys.time(), tz = "UTC", usetz = TRUE),
    source_endpoint  = "hoopR::mbb_schedule_crosswalk()"
  )
...
tryCatch({
  ...
  upload_mbb_manifest(
    manifest_path        = "mbb/crosswalk/mbb_schedule_crosswalk_in_data_repo.csv",
    release_tag          = "mbb_crosswalk",
    file_name            = "mbb_schedule_crosswalk_in_data_repo",
    sportsdataverse_type = "schedule crosswalk manifest",
    pkg_function         = "hoopR::load_mbb_schedule_crosswalk()"
  )
}, error = function(e) {
  cli::cli_alert_warning(
