...
raw_base <- "https://raw.githubusercontent.com/sportsdataverse/hoopR-mbb-raw/main/mbb/team_rosters/json"
gh_api_base <- "https://api.github.com/repos/sportsdataverse/hoopR-mbb-raw/contents/mbb/team_rosters/json"
...
build_season_rosters <- function(y) {
  ...
  cli::cli_progress_step(
    msg = "Compiling {y} ESPN MBB rosters ({length(team_ids)} teams)",
    msg_done = "Compiled {y} ESPN MBB rosters!"
  )
  ...
  rosters <- rosters %>%
    ...
    hoopR:::make_hoopR_data(
      "ESPN MBB Rosters from hoopR data repository",
      Sys.time()
    )

  ifelse(
    !dir.exists(file.path("mbb/rosters")),
    dir.create(file.path("mbb/rosters"), recursive = TRUE),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("mbb/rosters/rds")),
    dir.create(file.path("mbb/rosters/rds"), recursive = TRUE),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("mbb/rosters/parquet")),
    dir.create(file.path("mbb/rosters/parquet"), recursive = TRUE),
    FALSE
  )

  saveRDS(rosters, glue::glue("mbb/rosters/rds/rosters_{y}.rds"))
  arrow::write_parquet(
    rosters,
    glue::glue("mbb/rosters/parquet/rosters_{y}.parquet"),
    compression = "zstd",
    compression_level = 22
  )

  cli::cli_progress_step(
    msg = "Updating {y} ESPN MBB Rosters GitHub Release",
    msg_done = "Updated {y} ESPN MBB Rosters GitHub Release!"
  )
  ...
  purrr::insistently(
    ...
    release_tag = "espn_mens_college_basketball_rosters",
    pkg_function = "hoopR::load_mbb_rosters()",
    file_types = c("rds", "csv", "parquet"),
    .token = Sys.getenv("GITHUB_PAT")
  )

  # --- Manifest row append --------------------------------------------------
  manifest_path <- "mbb/rosters/nba_rosters_in_data_repo.csv"
...
tryCatch({
  ...
  upload_mbb_manifest(
    ...
    pkg_function         = "hoopR::load_mbb_rosters_manifest()"
  )
}, error = function(e) {
