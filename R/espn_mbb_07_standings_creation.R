...
raw_base <- "https://raw.githubusercontent.com/sportsdataverse/hoopR-mbb-raw/main/mbb/standings/json"
...
write_manifest_row <- function(season, row_count, source_endpoint) {
  manifest_path <- "mbb/standings/nba_standings_in_data_repo.csv"
  ifelse(
    !dir.exists(file.path("mbb/standings")),
    dir.create(file.path("mbb/standings"), recursive = TRUE),
    FALSE
  )
  row <- tibble::tibble(
...
build_season_standings <- function(y) {
  ...
  cli::cli_progress_step(
    msg = "Compiling {y} ESPN MBB standings",
    msg_done = "Compiled {y} ESPN MBB standings!"
  )
  ...
  standings <- standings %>%
    ...
    hoopR:::make_hoopR_data(
      "ESPN MBB Standings from hoopR data repository",
      Sys.time()
    )
  ...
  ifelse(
    !dir.exists(file.path("mbb/standings/rds")),
    dir.create(file.path("mbb/standings/rds"), recursive = TRUE),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("mbb/standings/parquet")),
    dir.create(file.path("mbb/standings/parquet"), recursive = TRUE),
    FALSE
  )

  saveRDS(standings, glue::glue("mbb/standings/rds/standings_{y}.rds"))
  arrow::write_parquet(
    standings,
    glue::glue("mbb/standings/parquet/standings_{y}.parquet"),
    compression = "zstd",
    compression_level = 22
  )

  cli::cli_progress_step(
    msg = "Updating {y} ESPN MBB Standings GitHub Release",
    msg_done = "Updated {y} ESPN MBB Standings GitHub Release!"
  )
  ...
  purrr::insistently(
    ...
    release_tag = "espn_mens_college_basketball_standings",
    pkg_function = "hoopR::load_mbb_standings()",
...
tryCatch({
  ...
  upload_mbb_manifest(
    ...
    pkg_function         = "hoopR::load_mbb_standings_manifest()"
  )
}, error = function(e) {
  cli::cli_alert_warning(
