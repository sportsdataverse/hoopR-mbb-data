...
build_season_team_crosswalk <- function(y) {
  cli::cli_progress_step(
    msg = "Compiling {y} MBB team crosswalk",
    msg_done = "Compiled {y} MBB team crosswalk!"
  )

  xwalk <- tryCatch(
    hoopR::mbb_team_crosswalk(season = y),
    error = function(e) {
  ...
  saveRDS(xwalk, glue::glue("mbb/crosswalk/rds/mbb_team_crosswalk_{y}.rds"))
  arrow::write_parquet(
    xwalk,
    glue::glue("mbb/crosswalk/parquet/mbb_team_crosswalk_{y}.parquet"),
    compression = "zstd",
    compression_level = 22
  )

  cli::cli_progress_step(
    msg = "Updating {y} MBB Team Crosswalk GitHub Release",
    msg_done = "Updated {y} MBB Team Crosswalk GitHub Release!"
  )
  ...
  purrr::insistently(
    ...
    data_frame = xwalk,
    file_name = glue::glue("mbb_team_crosswalk_{y}"),
    sportsdataverse_type = "team crosswalk data",
    release_tag = "mbb_crosswalk",
    pkg_function = "hoopR::mbb_team_crosswalk()",
    file_types = c("rds", "csv", "parquet"),
    .token = Sys.getenv("GITHUB_PAT")
  )

  # --- Manifest row append --------------------------------------------------
  manifest_path <- "mbb/crosswalk/mbb_team_crosswalk_in_data_repo.csv"
  manifest_row <- tibble::tibble(
    season           = as.integer(y),
    row_count        = as.integer(nrow(xwalk)),
    generated_at_utc = format(Sys.time(), tz = "UTC", usetz = TRUE),
    source_endpoint  = "hoopR::mbb_team_crosswalk()"
  )
...
tryCatch({
  ...
  upload_mbb_manifest(
    manifest_path        = "mbb/crosswalk/mbb_team_crosswalk_in_data_repo.csv",
    release_tag          = "mbb_crosswalk",
    file_name            = "mbb_team_crosswalk_in_data_repo",
    sportsdataverse_type = "team crosswalk manifest",
    pkg_function         = "hoopR::load_mbb_team_crosswalk()"
  )
}, error = function(e) {
  cli::cli_alert_warning(
