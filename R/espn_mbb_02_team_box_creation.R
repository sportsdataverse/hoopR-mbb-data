...
mbb_team_box_games <- function(y) {
  ...
  if (length(season_team_box_list) > 0) {
    cli::cli_progress_step(
      msg = "Compiling {y} ESPN MBB Team Boxscores ({length(season_team_box_list)} games)",
    ...
    espn_df <- furrr::future_map_dfr(
      ...
      function(x) {
        tryCatch(
          ...
          error = function(e) {
            message(glue::glue(
              "{Sys.time()}: game_id {x} team box issue with error {e}"
            ))
          }
        )
      },
    ...
    cli::cli_progress_step(
      msg = "Updating {y} ESPN MBB Team Boxscores GitHub Release",
  ...
  if (nrow(espn_df) > 0) {
    espn_df <- espn_df %>%
      ...
      hoopR:::make_hoopR_data(
        "ESPN MBB Team Boxscores from hoopR data repository",
        Sys.time()
      )

    ifelse(
      !dir.exists(file.path("mbb/team_box")),
      dir.create(file.path("mbb/team_box")),
      FALSE
    )
    ...
    ifelse(
      !dir.exists(file.path("mbb/team_box/rds")),
      dir.create(file.path("mbb/team_box/rds")),
      FALSE
    )
    saveRDS(espn_df, glue::glue("mbb/team_box/rds/team_box_{y}.rds"))

    ifelse(
      !dir.exists(file.path("mbb/team_box/parquet")),
      dir.create(file.path("mbb/team_box/parquet")),
      FALSE
    )
    arrow::write_parquet(
      espn_df,
      glue::glue("mbb/team_box/parquet/team_box_{y}.parquet")
    )
  ...
  if (nrow(espn_df) > 0) {
    ...
    cli::cli_alert_info(
      "{length(season_team_box_list)} ESPN MBB Team Boxscores to be compiled for {y}, skipping Team Boxscores compilation"
    )
