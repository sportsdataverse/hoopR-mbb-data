...
write_manifest_row <- function(season, row_count, source_endpoint) {
  manifest_path <- "mbb/game_rosters/nba_game_rosters_in_data_repo.csv"
  ifelse(
    !dir.exists(file.path("mbb/game_rosters")),
    dir.create(file.path("mbb/game_rosters"), recursive = TRUE),
    FALSE
  )
  row <- tibble::tibble(
    season = as.integer(season),
...
build_season_game_rosters <- function(y) {
  ...
  cli::cli_progress_step(
    msg = "Compiling {y} ESPN MBB game rosters ({length(game_ids)} games)",
    msg_done = "Compiled {y} ESPN MBB game rosters!"
  )
  ...
  rosters_df <- rosters_df %>%
    ...
    hoopR:::make_hoopR_data(
      "ESPN MBB Game Rosters from hoopR data repository",
      Sys.time()
    )
  ...
  ifelse(
    !dir.exists(file.path("mbb/game_rosters/rds")),
    dir.create(file.path("mbb/game_rosters/rds"), recursive = TRUE),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("mbb/game_rosters/parquet")),
    dir.create(file.path("mbb/game_rosters/parquet"), recursive = TRUE),
    FALSE
  )

  saveRDS(
    rosters_df,
    glue::glue("mbb/game_rosters/rds/game_rosters_{y}.rds")
  )
  arrow::write_parquet(
    rosters_df,
    glue::glue("mbb/game_rosters/parquet/game_rosters_{y}.parquet")
  )

  cli::cli_progress_step(
    msg = "Updating {y} ESPN MBB Game Rosters GitHub Release",
    msg_done = "Updated {y} ESPN MBB Game Rosters GitHub Release!"
  )
  ...
  purrr::insistently(
    ...
    release_tag = "espn_mens_college_basketball_game_rosters",
...
tryCatch({
  ...
  upload_mbb_manifest(
    ...
    pkg_function         = "hoopR::load_mbb_game_rosters_manifest()"
  )
}, error = function(e) {
  cli::cli_alert_warning(
