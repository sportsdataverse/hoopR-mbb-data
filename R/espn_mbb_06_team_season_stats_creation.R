...
raw_base <- "https://raw.githubusercontent.com/sportsdataverse/hoopR-mbb-raw/main/mbb/team_stats/json"
gh_api_base <- "https://api.github.com/repos/sportsdataverse/hoopR-mbb-raw/contents/mbb/team_stats/json"
...
write_manifest_row <- function(season, row_count, source_endpoint) {
  manifest_path <- "mbb/team_season_stats/nba_team_season_stats_in_data_repo.csv"
  ifelse(
    !dir.exists(file.path("mbb/team_season_stats")),
    dir.create(file.path("mbb/team_season_stats"), recursive = TRUE),
    FALSE
  )
  row <- tibble::tibble(
...
build_season_team_stats <- function(y) {
  ...
  cli::cli_progress_step(
    msg = "Compiling {y} ESPN MBB team season stats ({length(team_ids)} teams)",
    msg_done = "Compiled {y} ESPN MBB team season stats!"
  )
  ...
  stats <- stats %>%
    ...
    hoopR:::make_hoopR_data(
      "ESPN MBB Team Season Stats from hoopR data repository",
      Sys.time()
    )
  ...
  ifelse(
    !dir.exists(file.path("mbb/team_season_stats/rds")),
    dir.create(file.path("mbb/team_season_stats/rds"), recursive = TRUE),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("mbb/team_season_stats/parquet")),
    dir.create(
      file.path("mbb/team_season_stats/parquet"),
      recursive = TRUE
    ),
    FALSE
  )
  ...
  saveRDS(
    stats,
    glue::glue("mbb/team_season_stats/rds/team_season_stats_{y}.rds")
  )
  arrow::write_parquet(
    stats,
    glue::glue(
      "mbb/team_season_stats/parquet/team_season_stats_{y}.parquet"
    ),
    compression = "zstd",
  ...
  cli::cli_progress_step(
    msg = "Updating {y} ESPN MBB Team Season Stats GitHub Release",
    msg_done = "Updated {y} ESPN MBB Team Season Stats GitHub Release!"
  )
  ...
  purrr::insistently(
    ...
    release_tag = "espn_mens_college_basketball_team_season_stats",
    pkg_function = "hoopR::load_mbb_team_stats()",
...
tryCatch({
  ...
  upload_mbb_manifest(
    ...
    pkg_function         = "hoopR::load_mbb_team_stats_manifest()"
  )
}, error = function(e) {
  cli::cli_alert_warning(
