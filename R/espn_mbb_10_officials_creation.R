...
raw_base <- "https://raw.githubusercontent.com/sportsdataverse/hoopR-mbb-raw/main/mbb/game_rosters/json"
sched_base <- "https://raw.githubusercontent.com/sportsdataverse/hoopR-mbb-raw/main/mbb/schedules/rds"
...
parse_one_game <- function(season, game_id) {
  ...
  # The ESPN site-API summary carries on-ice/on-court officials under
  # gameInfo.officials -- a flat list of {fullName, displayName, position{...},
  # order}. Reuse the per-game summary already cached for game_rosters rather
  # than the core-API event_officials endpoint (which is $ref-paginated).
  officials <- raw[["gameInfo"]][["officials"]] %||% list()
  if (length(officials) == 0) return(NULL)

  purrr::map_dfr(officials, function(off) {
    pos <- off[["position"]] %||% list()
    tibble::tibble(
      season                = as.integer(season),
      game_id               = safe_int(game_id),
      official_full_name    = safe_chr(off[["fullName"]]),
      official_display_name = safe_chr(off[["displayName"]]) %|%
        safe_chr(off[["fullName"]]),
      official_position      = safe_chr(pos[["name"]]) %|%
        safe_chr(pos[["displayName"]]),
      official_position_id   = safe_int(pos[["id"]]),
      official_order         = safe_int(off[["order"]])
    )
  })
}

write_manifest_row <- function(season, row_count, source_endpoint) {
  manifest_path <- "mbb/officials/nba_officials_in_data_repo.csv"
  ifelse(
    !dir.exists(file.path("mbb/officials")),
    dir.create(file.path("mbb/officials"), recursive = TRUE),
    FALSE
  )
  row <- tibble::tibble(
    season = as.integer(season),
...
build_season_officials <- function(y) {
  ...
  cli::cli_progress_step(
    msg = "Compiling {y} ESPN MBB officials ({length(game_ids)} games)",
    msg_done = "Compiled {y} ESPN MBB officials!"
  )
  ...
  rosters_df <- rosters_df %>%
    ...
    hoopR:::make_hoopR_data(
      "ESPN MBB Officials from hoopR data repository",
      Sys.time()
    )
  ...
  ifelse(
    !dir.exists(file.path("mbb/officials/rds")),
    dir.create(file.path("mbb/officials/rds"), recursive = TRUE),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("mbb/officials/parquet")),
    dir.create(file.path("mbb/officials/parquet"), recursive = TRUE),
    FALSE
  )

  saveRDS(
    rosters_df,
    glue::glue("mbb/officials/rds/officials_{y}.rds")
  )
  arrow::write_parquet(
    rosters_df,
    glue::glue("mbb/officials/parquet/officials_{y}.parquet")
  )

  cli::cli_progress_step(
    msg = "Updating {y} ESPN MBB Officials GitHub Release",
    msg_done = "Updated {y} ESPN MBB Officials GitHub Release!"
  )
  ...
  purrr::insistently(
    ...
    release_tag = "espn_mens_college_basketball_officials",
...
tryCatch({
  ...
  upload_mbb_manifest(
    manifest_path        = "mbb/officials/nba_officials_in_data_repo.csv",
    release_tag          = "espn_mens_college_basketball_officials",
    file_name            = "nba_officials_in_data_repo",
    sportsdataverse_type = "officials manifest",
    pkg_function         = "hoopR::load_mbb_officials_manifest()"
  )
}, error = function(e) {
  cli::cli_alert_warning(
