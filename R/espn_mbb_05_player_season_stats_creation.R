...
# Athlete-keyed raw payloads: mbb/player_season_stats/json/{athlete_id}.json.
...
raw_base <- "https://raw.githubusercontent.com/sportsdataverse/hoopR-mbb-raw/main/mbb/player_season_stats/json"
...
# espn_mens_college_basketball_player_boxscores release -- the authoritative "who played in Y"
...
build_identity_lookup <- function(season) {
  pb <- tryCatch(
    hoopR::load_mbb_player_box(seasons = season),
    error = function(e) {
...
build_season_player_stats <- function(y) {
  ...
  cli::cli_progress_step(
    msg = "Compiling {y} ESPN MBB player season stats ({length(athlete_ids)} athletes)",
    msg_done = "Compiled {y} ESPN MBB player season stats!"
  )
  ...
  stats <- stats %>%
    ...
    hoopR:::make_hoopR_data(
      "ESPN MBB Player Season Stats from hoopR data repository",
      Sys.time()
    )

  ifelse(
    !dir.exists(file.path("mbb/player_season_stats")),
    dir.create(file.path("mbb/player_season_stats"), recursive = TRUE),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("mbb/player_season_stats/rds")),
    dir.create(file.path("mbb/player_season_stats/rds"), recursive = TRUE),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("mbb/player_season_stats/parquet")),
    dir.create(file.path("mbb/player_season_stats/parquet"), recursive = TRUE),
    FALSE
  )

  saveRDS(
    stats,
    glue::glue("mbb/player_season_stats/rds/player_season_stats_{y}.rds")
  )
  arrow::write_parquet(
    stats,
    glue::glue("mbb/player_season_stats/parquet/player_season_stats_{y}.parquet")
  )

  cli::cli_progress_step(
    msg = "Updating {y} ESPN MBB Player Season Stats GitHub Release",
    msg_done = "Updated {y} ESPN MBB Player Season Stats GitHub Release!"
  )
  ...
  purrr::insistently(
    ...
    release_tag = "espn_mens_college_basketball_player_season_stats",
    pkg_function = "hoopR::load_mbb_player_stats()",
    file_types = c("rds", "csv", "parquet"),
    .token = Sys.getenv("GITHUB_PAT")
  )

  # --- Manifest row append --------------------------------------------------
  manifest_path <- "mbb/player_season_stats/nba_player_season_stats_in_data_repo.csv"
...
tryCatch({
  ...
  upload_mbb_manifest(
    ...
    pkg_function         = "hoopR::load_mbb_player_stats_manifest()"
  )
}, error = function(e) {
  cli::cli_alert_warning(
