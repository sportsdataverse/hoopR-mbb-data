...
# Stage 16 -- player_core. Twin of python/espn_mbb_16_player_core_creation.py.
#
...
raw_base <- "https://raw.githubusercontent.com/sportsdataverse/hoopR-mbb-raw/main/mbb/player_core/json"
...
build_season_player_core <- function(y) {
  ...
  cli::cli_progress_step(
    msg = "Compiling {y} ESPN MBB player core ({length(athlete_ids)} athletes)",
    msg_done = "Compiled {y} ESPN MBB player core!"
  )
  ...
  core <- core %>%
    ...
    hoopR:::make_hoopR_data(
      "ESPN MBB Player Core from hoopR data repository",
      Sys.time()
    )

  for (d in c("mbb/player_core", "mbb/player_core/rds", "mbb/player_core/parquet")) {
    if (!dir.exists(d)) dir.create(d, recursive = TRUE)
  }

  saveRDS(core, glue::glue("mbb/player_core/rds/player_core_{y}.rds"))
  arrow::write_parquet(
    core,
    glue::glue("mbb/player_core/parquet/player_core_{y}.parquet")
  )

  cli::cli_progress_step(
    msg = "Updating {y} ESPN MBB Player Core GitHub Release",
    msg_done = "Updated {y} ESPN MBB Player Core GitHub Release!"
  )
  ...
  purrr::insistently(
    ...
    release_tag = "espn_mbb_player_core",
    ...
    pkg_function = "sportsdataverse-data release espn_mbb_player_core",
    file_types = c("rds", "csv", "parquet"),
    .token = Sys.getenv("GITHUB_PAT")
  )

  manifest_path <- "mbb/player_core/nba_player_core_in_data_repo.csv"
